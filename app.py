...
@st.cache_resource
def load_artefacts():
    model               = tf.keras.models.load_model("deepcsat_model.keras")
...
with c1:
    ...
    ax.bar(["Email", "Inbound", "Outcall"], [26.5, 17.3, 16.8],
           color=["#e74c3c", "#f39c12", "#27ae60"], edgecolor="none", width=0.5)
    ax.set_ylabel("Dissatisfied %", fontsize=9); ax.set_ylim(0, 35)
    ax.spines[["top", "right", "left"]].set_visible(False)
...
with c2:
    ...
    colors_sh = ["#27ae60" if s=="Split" else "#e74c3c" if s=="Morning" else "#3498db" for s in shifts]
    ax.barh(shifts, scores, color=colors_sh, edgecolor="none", height=0.45)
    ax.set_xlim(4.1, 4.55); ax.set_xlabel("Avg CSAT", fontsize=9)
...
Visit"]
    ...
    ax.set_ylim(0, 5); ax.set_ylabel("Avg CSAT", fontsize=9)
